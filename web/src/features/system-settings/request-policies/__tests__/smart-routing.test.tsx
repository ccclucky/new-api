...
beforeEach(() => {
  ...
  currentOptions = {
    'auto_routing_setting.enabled': 'false',
    'auto_routing_setting.virtual_model': 'auto',
    'auto_routing_setting.base_url': 'https://api.typesafe.ai',
    'auto_routing_setting.api_key': 'sk-test',
    'auto_routing_setting.fallback_model': 'gpt-5.1',
    'auto_routing_setting.timeout_ms': '2000',
  }
...
it('saves only the changed decision timeout', async () => {
  ...
  expect(vi.mocked(api.put).mock.calls[0][1]).toEqual({
    key: 'auto_routing_setting.timeout_ms',
    value: '500',
  })
  expect(api.put).not.toHaveBeenCalledWith(
    '/api/option/',
    expect.objectContaining({ key: 'auto_routing_setting.api_key' })
  )
})
